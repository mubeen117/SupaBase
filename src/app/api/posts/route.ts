...
export async function GET(request: Request) {
  try {
    ...
    const db = await createConnection();
    const sql = id ? "SELECT * FROM scanner WHERE id = $1" : "SELECT * FROM scanner";
    const values = id ? [id] : [];
...
export async function PUT(request: Request) {
  try {
    ...
    const sql = `UPDATE scanner SET SAP_ID = $1, productName = $2, Date = $3, qr_code = $4 WHERE ID = $5`;
    const values = [SAP_ID, productName, Date, qr_code, ID];
